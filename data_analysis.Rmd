...
```{r}
protest_daily_2020 <- protest_data_full %>% 
  filter(weekly == 0, large == 1,
         liberal == 0, conservative == 0, 
         onlyprotest == 0, norallies == 0, salient == 0) %>% 
  mutate(date = as.Date(date)) %>% 
  filter(between(date, as.Date("2020-05-04"), as.Date("2020-06-15"))) %>% 
  group_by(date) %>% 
  summarise(n_total = sum(n))

donations_plot <- amount_daily_donations_full %>% 
  filter(days > -22 & days < 22) %>% 
  mutate(days = as.factor(days),
         after = as.factor(after))

plot <- cbind(protest_daily_2020, donations_plot)

plot %>% 
  ggplot() +
  geom_point(aes(x = days, 
                 y = amount_donations)) +
  geom_col(aes(x = as.numeric(days), y = n_total*54660)) +
  geom_smooth(aes(x = as.numeric(days),
                  y =  amount_donations,
                  linetype = after),
              size = 1, color = "red") +
  geom_vline(xintercept = which(levels(plot$days) %in% '0'), 
             linetype= "longdash") +
  theme(legend.position = "none") +
  labs(title = "Federal Campaign Contributions and Political Protests") +
  scale_x_discrete(breaks = seq(-21, 21, by = 3), 
                   name = "Days since Murder of George Floyd") +
  ...
  theme(axis.text.x = element_text(hjust = 1,
                                   angle = 60)) +
  geom_label(
    label="# of Daily Protests", 
    x=which(levels(plot$days) %in% '9'),
    y=6000000,
  ...
  geom_label(
    label="Highest # of Daily Protests = 472", 
    x=which(levels(plot$days) %in% '12'),
    y=31000000,
    label.padding = unit(0.1, "lines"), # Rectangle size around label
    label.size = NA,
    color = "black",
  ...
  annotate("segment", x = which(levels(plot$days) %in% '12'), 
           y = 30000000, xend = which(levels(plot$days) %in% '12'), 
           yend = 26600000, 
         size = 2, linejoin = "mitre",color = "#56B4E9",
...
summary(lout_optimal_all)
# Full Data - Amount (optimal) LOG
lout_optimal_all <- lm(log(amount_donations) ~ days + I(days > 0), data = bw_amount_dta)
summary(lout_optimal_all)
...
```
Number of significant coefficients is 23 out of 141. Number of positive significant coefficients 10 of of 141. Mean coefficient is 382188 and median is 1741858. Mean p-value is 0.353 and the median p-value is 0.401.
...
### Robustness Check: Poisson and Negative Binomial
...
#### African American Candidates v. White Candidates
...
dem_coefficients <- summary(lout_optimal_dem_std)[["coefficients"]][3, 1:2]
rep_coefficients <- summary(lout_optimal_rep_std)[["coefficients"]][3, 1:2]
inc_coefficients <- summary(lout_optimal_inc_std)[["coefficients"]][3, 1:2]
chall_coefficients <- summary(lout_optimal_chall_std)[["coefficients"]][3, 1:2]
aa_coefficients <- summary(lout_optimal_aa_std)[["coefficients"]][3, 1:2]
white_coefficients <- summary(lout_optimal_white_std)[["coefficients"]][3, 1:2]

subgroup_plot <- rbind(dem_coefficients, rep_coefficients, inc_coefficients,
                      chall_coefficients, aa_coefficients, white_coefficients)

subgroup_plot <- rownames_to_column(as.data.frame(subgroup_plot), "model")

colnames(subgroup_plot) <- c("model", "estimate", "se")

subgroup_plot <- subgroup_plot %>% 
  mutate(model = dplyr::recode(model, "dem_coefficients" = "Democrats Only",
                               "rep_coefficients" = "Republicans Only",
                               "inc_coefficients" = "Incumbents Only",
                               "chall_coefficients" = "Challengers Only",
                               "aa_coefficients" = "African Americans Only",
                               "white_coefficients" = "Whites Only"),
         model = factor(model, levels = c("Democrats Only",
                                          "Republicans Only",
                                          "Incumbents Only",
                                          "Challengers Only",
...
subgroup_plot %>% 
  ...
  labs(title = "Effect of Political Protests on Donations to Subsets of Candidates",
       caption = "Note: Each model is fit using the Imbens-Kalyanaraman Optimal Bandwidth.") +
  ylab("Standardized Increase in Donations") +
  xlab("Candidate Subgroup")
```
...
### Diff-in-Diff Regression Tables

```{r}
# Protests of more than 30 people

##### 1) Analysis on full data
data_standard <- data_large[[1]]
# dv: amount of donations
model1_amount <- run_regression(data_standard, FALSE, FALSE)

### 2) Analysis on data with lagged DV and IV --- MAIN MODEL
data_lag <- data_large[[4]]
# dv: amount of donations
model2_amount <- summary(run_regression(data_lag, FALSE, TRUE))

### 3) Analysis on data with lagged DV and IV and log of DV
data_lag_logDV <- data_large[[5]]
# dv: amount of donations
model3_amount <- run_regression(data_lag_logDV, FALSE, TRUE)

# etable(model1_amount,
#           model2_amount,
#           model3_amount,
#           tex = TRUE)
...
##### 1) Analysis on full data
data_dem_standard <- data_dem[[1]]
# dv: amount of donations
model1_dem_amount <- run_regression(data_dem_standard, FALSE, FALSE)
...
### 3) Analysis on data with lagged DV and IV and log of DV
data_lag_dem_logDV <- data_dem[[5]]
# dv: amount of donations
model3_dem_amount <- run_regression(data_lag_dem_logDV, FALSE, TRUE)

# etable(model1_dem_amount,
#           model2_dem_amount,
#           model3_dem_amount,
#           tex = TRUE)
...
##### 1) Analysis on full data
data_dem_rep_standard <- data_dem_rep[[1]]
# dv: amount of donations
model1_dem_rep_amount <- run_regression(data_dem_rep_standard, FALSE, FALSE)
...
### 3) Analysis on data with lagged DV and IV and log of DV
data_lag_dem_rep_logDV <- data_dem_rep[[5]]
# dv: amount of donations
model3_dem_rep_amount <- run_regression(data_lag_dem_rep_logDV, FALSE, TRUE)

# etable(model1_dem_rep_amount,
#           model2_dem_rep_amount,
#           model3_dem_rep_amount,
#           tex = TRUE)
...
##### 1) Analysis on full data
data_rep_standard <- data_rep[[1]]
# dv: amount of donations
model1_rep_amount <- run_regression(data_rep_standard, FALSE, FALSE)
...
### 3) Analysis on data with lagged DV and IV and log of DV
data_lag_rep_logDV <- data_rep[[5]]
# dv: amount of donations
model3_rep_amount <- run_regression(data_lag_rep_logDV, FALSE, TRUE)

# etable(model1_rep_amount,
#           model2_rep_amount,
#           model3_rep_amount,
#           tex = TRUE)
...
##### 1) Analysis on full data
data_rep_dem_standard <- data_rep_dem[[1]]
# dv: amount of donations
model1_rep_dem_amount <- run_regression(data_rep_dem_standard, FALSE, FALSE)
...
### 3) Analysis on data with lagged DV and IV and log of DV
data_lag_rep_dem_logDV <- data_rep_dem[[5]]
# dv: amount of donations
model3_rep_dem_amount <- run_regression(data_lag_rep_dem_logDV, FALSE, TRUE)

# etable(model1_rep_dem_amount,
#           model2_rep_dem_amount,
#           model3_rep_dem_amount,
#           tex = TRUE)
